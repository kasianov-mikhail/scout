...
@MainActor
@Suite("VersionEntry+Monitor")
struct VersionEntryMonitorTests {
    ...
    private func seedCurrentLaunch() {
        ...
        LaunchEntry.stub(date: Date(), install: install, in: context)
    }

    @Test("trigger creates a VersionEntry on first call")
    func firstTrigger() throws {
        seedCurrentLaunch()
        try VersionEntry.Trigger(
            installID: identity.install,
            launchID: identity.launch,
            bundle: .stub(appVersion: "1.0", buildNumber: "1")
        ).execute(in: context)

        let versions = try context.fetchAll(VersionEntry.self)
    ...
    @Test("trigger creates a new VersionEntry when appVersion changes")
    ...
    @Test("trigger creates a new VersionEntry when buildNumber changes")
    ...
    @Test("trigger ignores VersionEntrys from other installs")
    func ignoresOtherInstalls() throws {
        let otherInstall = InstallEntry.stub(date: Date(), in: context)
        otherInstall.installID = UUID()

        let otherLaunch = LaunchEntry.stub(date: Date(), install: otherInstall, in: context)
        otherLaunch.launchID = UUID()

        VersionEntry.stub(date: Date(), appVersion: "1.0", buildNumber: "1", launch: otherLaunch, in: context)
        try context.save()
