...
struct VersionObjectMonitorTests {
    ...
    func firstTrigger() throws {
        seedCurrentLaunch()
        try VersionObject.trigger(identity: identity, appVersion: "1.0", buildNumber: "1", in: context)
    ...
    func appVersionChange() throws {
        seedCurrentLaunch()
        try VersionObject.trigger(identity: identity, appVersion: "1.0", buildNumber: "1", in: context)
        try VersionObject.trigger(identity: identity, appVersion: "2.0", buildNumber: "1", in: context)
    ...
    func buildNumberChange() throws {
        seedCurrentLaunch()
        try VersionObject.trigger(identity: identity, appVersion: "1.0", buildNumber: "1", in: context)
        try VersionObject.trigger(identity: identity, appVersion: "1.0", buildNumber: "2", in: context)
