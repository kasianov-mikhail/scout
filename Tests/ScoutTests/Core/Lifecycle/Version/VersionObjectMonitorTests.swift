...
struct VersionObjectMonitorTests {
    ...
    func firstTrigger() throws {
        seedCurrentLaunch()
        try VersionObject.Trigger(installID: identity.install, launchID: identity.launch, bundle: .stub(appVersion: "1.0", buildNumber: "1")).execute(in: context)
    ...
    func appVersionChange() throws {
        seedCurrentLaunch()
        try VersionObject.Trigger(installID: identity.install, launchID: identity.launch, bundle: .stub(appVersion: "1.0", buildNumber: "1")).execute(in: context)
        try VersionObject.Trigger(installID: identity.install, launchID: identity.launch, bundle: .stub(appVersion: "2.0", buildNumber: "1")).execute(in: context)
    ...
    func buildNumberChange() throws {
        seedCurrentLaunch()
        try VersionObject.Trigger(installID: identity.install, launchID: identity.launch, bundle: .stub(appVersion: "1.0", buildNumber: "1")).execute(in: context)
        try VersionObject.Trigger(installID: identity.install, launchID: identity.launch, bundle: .stub(appVersion: "1.0", buildNumber: "2")).execute(in: context)
