...
extension Collection where Element: ChartPointProtocol {
    ...
    /// Use to group chart points into contiguous date buckets and sum their counts.
    ///
    /// Use with a date `range` and calendar `component` to produce one result per
    /// interval. Each result uses the interval’s start date and the aggregated
